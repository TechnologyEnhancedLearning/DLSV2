...
BEGIN
	...
	SELECT ca.ID AS CandidateAssessmentID, 
		...
		WHERE (NOT (ca.CompleteByDate IS NULL)) AND 
			(ca.CompleteByDate BETWEEN DATEADD(M, -1, GETDATE()) AND GETDATE()) AND 
	...
	--the email prefix and other variables will be the same for each e-mail so set them before we loop:
		...
		SET @_Subject = N'REMINDER: Your self assessment completion is overdue'
	...
	While exists (Select * From @Reminders) 
		...
		'<p>This is an automated reminder message from the Digital Learning Solutions platform to remind you that completion of the self assessment <b>' + SelfAssessment + N'</b> is <b>overdue</b>. The date the self assessment should have been completed by was ' + CONVERT(VARCHAR(10), CompleteByDate, 103) + N'.</p>'+
		...
		if @_EmailCC <> ''
		begin
			SET @bodyHTML = @bodyHTML + '<p><b>Note:</b> This message has been copied to the administrator who enrolled you on this self assessment for their information.</p>'
		end
		--Now send the e-mail:
