namespace DigitalLearningSolutions.Web.Controllers.SupervisorController
{
    ...
    public partial class SupervisorController
    {
        ...
        {
            var sessionEnrolOnCompetencyAssessment = multiPageFormService.GetMultiPageFormData<SessionEnrolOnCompetencyAssessment>(
                MultiPageFormDataFeature.EnrolDelegateOnProfileAssessment,
                TempData
            ).GetAwaiter().GetResult();
            multiPageFormService.SetMultiPageFormData(
        ...
        public async Task<IActionResult> DownloadCertificate(int candidateAssessmentId)
        {
            ...
            if (pdfReportResponse != null)
            {
                ...
                if (pdfReportFile != null)
                {
                    ...
                    var fileName = $"{shortSelfAssessment} - {model.CompetencySelfAssessmentCertificates.LearnerName.Substring(0, nameTextLength >= 15 ? 15 : nameTextLength)}" + (isPrnExist ? $" - {model.CompetencySelfAssessmentCertificates.LearnerPRN}.pdf" : ".pdf");
